...
def ipls(intervals, x_train, x_test, y_train, y_test):
    ...
    for i in range(1, intervals + 1):
        ...
        for nc in ncomp:
           ...
           # print("训练集决定系数为:", r2_score(y_test, y_test_pred))
    ...
    plt.bar(np.arange(1, intervals + 1), mse, width=0.5, color='r', linewidth=0.4)
...
# 1.数据获取
excelFile = pd.read_excel("F:\华南农业大学龙老师组\茶叶\茶叶茶多酚\实验项目\数据\dtest\可见+近红外\氨基酸红外_test.xlsx", header=None)#正11_转置
...
# 2.样本集划分
x_train, x_test, y_train, y_test = spxy(x, y, test_size=0.2)
