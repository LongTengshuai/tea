...
class SPA:
    ...
    def spa(self, Xcal, ycal, m_min=1, m_max=None, Xval=None, yval=None, autoscaling=1):
        ...
        fig1 = plt.figure()
        plt.xlabel('Number of variables included in the model',fontsize=18)
        ...
        #print('i_crit:'+str(np.RMSEP_scree[i_crit]))
        plt.grid(True)

        fig2 = plt.figure()
        plt.plot(Xcal[0, :])
        plt.scatter(var_sel, Xcal[0, var_sel], marker='s', color='r')
       # print('var_sel:'+var_sel)
        plt.legend(['First calibration object', 'Selected variables'],fontsize=18)
...
if __name__ == "__main__":
    excelFile = pd.read_excel("F:\华南农业大学龙老师组\茶叶\茶叶茶多酚\实验项目\数据\dtest\可见+近红外\波段\茶多酚34ipls_test.xlsx", header=None)
   ...
   # data = pd.read_excel(r"all.xlsx")
    ...
    #absorbances = featrues.columns

    from sklearn.model_selection import train_test_split
    from sklearn.preprocessing import MinMaxScaler

    Xcal, Xval, ycal, yval = train_test_split(featrues, category, test_size=0.4, random_state=0)
